...
class SinglePanel extends React.Component {
  ...
  constructor(props) {
    ...
    this.props.allPanelArr.current
  ...
  moveUp = () => {
    ...
    if (this.moveUpEnabled) {
      ...
      this.setState((state) => {
        return {
          panelTransY: state.panelTransY - 80,
        }
      })
    }
  }
...
function SinglePanel_({ item, setMainEnabled, setListRefEnabled, mainRef, listRef, scrollY, mainEnabled, bgColor,
  ...
  function settingMovePermission() {
    allPanelArr.current.forEach((panel) => {
      ...
      else if (panel.panelIndex > panelIndex) {
  ...
  function checkingMovement() {
    ...
    allPanelArr.current.forEach((panel) => {
      ...
      if (panel.state.panelIndex === panelIndex) { return }
      if (transY.value <= ((panel.state.panelIndex - panelIndex) * 2 - ((panel.state.panelIndex - panelIndex) > 0 ? 1 : -1)) * 40) { panel.moveDown() }
      else { panel.moveUp() }
  ...
  function reSortList(value) {
    ...
    // this.props.setMainEnabled(true)
    ...
    moveArr(allPanelArr.current, from, to)
