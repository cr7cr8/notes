  ...
  return (
    <>
      ...
      <Overlay isVisible={overLayOn} onBackdropPress={function () { setOverLayOn(false) }} >
        ...
        <Button title="ssds" onPress={async function (props) {
          ...
          const { status } = await FileSystem.downloadAsync(uri, fileUri, { headers: { token: "hihihi" } }) //.catch(e => { console.log(e) })


          if (status == 200) {
            const { granted } = await MediaLibrary.requestPermissionsAsync().catch(e => { console.log(e) }).catch(e => { console.log(e) })
            if (!granted) { return }
            else { movingFile(fileUri).catch(e => { console.log(e) }) }
          }
          ...
          //         movingFile(fileUri).catch(e => { console.log(e) })
          //         ///
          //         // (async function () {
          //         //   console.log("fdsf")
...
async function movingFile(fileUri) {
  console.log("fdsf")
  const asset = await MediaLibrary.createAssetAsync(fileUri);
  const album = await MediaLibrary.getAlbumAsync('ttt');
